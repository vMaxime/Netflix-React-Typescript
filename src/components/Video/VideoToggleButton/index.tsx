import { FC, ButtonHTMLAttributes, SVGProps, useState } from 'react';

const VideoToggleButton: FC<ButtonHTMLAttributes<HTMLButtonElement>> = ({ className, ...props }) => {
    const [sound, setSound] = useState<boolean>(true);

    return (
        <button className={'video-action-button' + (!!className ? ' ' + className : '')} { ...props }>
            {
                sound ?
                <SoundOnSVG className="md:w-4 md:h-4 lg:w-5 lg:h-5 xl:w-6 xl:h-6 fill-white" onClick={() => setSound(false)} />
                :
                <SoundOffSVG className="md:w-4 md:h-4 lg:w-5 lg:h-5 xl:w-6 xl:h-6 fill-white" onClick={() => setSound(true)} />
            }
        </button>
    );
};

const SoundOffSVG: FC<SVGProps<SVGSVGElement>> = (props) => (
