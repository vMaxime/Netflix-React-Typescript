...
const HomeHeader: FC<HomeHeaderProps> = ({ profile }) => {
    ...
    return (
        <header className="fixed w-full z-50" style={{ backgroundColor, transition: `background-color ${backgroundColor === 'transparent' ? 1 : .3}s` }}>
            <div className="h-10 md:h-20 mx-auto px-4p flex gap-10">
                <a href="/" className="flex items-center">
                    <svg className="fill-red-600 w-12 md:w-24" width="95" viewBox="0 0 111 30" aria-hidden="true" focusable="false">
                ...
                <nav className="flex items-center gap-2 md:gap-5 flex-1 text-neutral-200">
                    <Link to="/" className="hidden lg:block text-sm lg:text-base">Accueil</Link>
                    <Link to="series" className="hidden lg:block text-sm lg:text-base">Séries</Link>
                    <Link to="films" className="hidden lg:block text-sm lg:text-base">Films</Link>
                    <Link to="news" className="hidden lg:block text-sm lg:text-base">Nouveautés les plus regardées</Link>
                    <Link to="my-list" className="hidden lg:block text-sm lg:text-base">Ma liste</Link>
                    <Link to="by-lang" className="hidden lg:block text-sm lg:text-base">Explorer par langue</Link>
                    <SearchBar className="hidden lg:block ml-auto" onCancel={() => {}} />
                    <Dropdown
                        icon={
                            <button role="dialog" className="ml-auto lg:ml-0 hover:cursor-pointer">
                                <svg className="h-4 w-4 sm:h-5 sm:w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 fill-white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <Dropdown
                        icon={
                            <button role="dialog" className="hover:cursor-pointer relative mr-3 md:mr-5">
                                <img src={profile.picture} className="w-4 h-4 sm:h-5 sm:w-5 md:h-7 md:w-7 lg:h-8 lg:w-8 rounded-sm md:rounded-md" alt="Profile picture" />
                                <svg className="h-2 w-2 md:h-3 md:w-3 chevron m-auto absolute top-1/3 -right-3 md:-right-5 fill-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512">
