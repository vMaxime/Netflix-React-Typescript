...
const HomeHeader: FC<HomeHeaderProps> = ({ profile }) => {
    ...
    const handleScroll = () => {
        if (backgroundColor === 'transparent' && window.scrollY > 0)
            setBackgroundColor('#141414');
        else if (backgroundColor === '#141414' && window.scrollY === 0)
    ...
    return (
        <header className="fixed w-full z-50" style={{ backgroundColor }}>
            <div className="h-20 mx-auto px-4p flex gap-10">
                ...
                <nav className="flex items-center gap-5 flex-1">
                    ...
                    >
                        {
                            ...
                            ?
                            <div className="py-10 text-center text-dark hover:bg-black flex-1 select-none">
                    ...
                    <Dropdown
                        showOnHover={true}
                        icon={<img src={profile.picture} className="w-8 h-8 rounded-md" alt="Profile picture" />}
                        carpet={true}
                        width="250px"
                        border={false}
                    >
                        <div className="w-full py-2">
                        {
                            profiles.filter(profile => selectedProfile != null && profile.id != selectedProfile).map(profile =>
                                <button key={profile.id} className="w-full flex items-center gap-3 py-2 px-2 hover:underline" onClick={() => selectProfile(profile)}>
                                    <div className="flex w-9 h-9">
                                        <img src={ profile.picture } className="w-9 h-9 rounded-md" alt="Profile picture" />
                                    </div>
