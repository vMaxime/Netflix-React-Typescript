...
const HomeHeader: FC<HomeHeaderProps> = ({ profile }) => {
    ...
    const navUrls: string[][] = [
        ...
        ['series', 'Séries'],
        ['films', 'Films'],
        ['news', 'Nouveautés les plus regardées'],
        ['my-list', 'Ma liste'],
        ['by-lang', 'Explorer par langue'],
    ];
        ...
        <header className="fixed w-full z-50" style={{ backgroundColor, transition: `background-color ${backgroundColor === 'transparent' ? 1 : .3}s` }}>
            <div className="h-12 md:h-20 mx-auto px-4p flex gap-5 lg:gap-10">
                        ...
                        <ul className="flex flex-col text-md">
                            {
                                navUrls.map(([path, label]) => 
                                    <li key={path} className="text-start"><Link to={ path }>{ label }</Link></li>
                                )
                            }
                        </ul>
                    </nav>
                ...
                <nav className="flex items-center gap-2 md:gap-5 flex-1 text-neutral-200">
                    <ul className="hidden lg:flex items-center gap-2 md:gap-4 xl:gap-5 text-md xl:text-base">
                        {
                            navUrls.map(([path, label]) => 
                                <li key={path}><Link to={ path } className="hidden lg:block">{ label }</Link></li>
                            )
                        }
                    </ul>
                    <SearchBar
                        ...
                        icon={
                            <button>
                                <svg className="h-4 w-4 md:w-6 md:h-6" viewBox="0 0 24 24" fill="#FFF" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <Dropdown
                        icon={
                            <div tabIndex={0} className="hidden lg:block hover:cursor-pointer">
                                <svg className="h-5 w-5 md:h-6 md:w-6 lg:h-7 lg:w-7 fill-white" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                    ...
                    <Dropdown
                        icon={
                            <div tabIndex={0} className="hidden lg:block hover:cursor-pointer relative mr-3 md:mr-5">
                                <img src={profile.picture} className="h-5 w-5 md:h-7 md:w-7 lg:h-8 lg:w-8 rounded-sm md:rounded-md" alt="Profile picture" />
