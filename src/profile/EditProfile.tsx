...
const EditProfile: React.FC<EditProfileProps> = ({ profile, handleCancel, onEditPicture, onSaved }) => {
    const [error, setError] = useState<string | null>(null);
    const user = useContext(UserContext);
    const profiles = user?.profiles || [];
    const [newProfile, setNewProfile] = useState<ProfileInterface>(profiles.find(({ name }) => name === profile.name) || profile);
    const dispatch = useContext(UserDispatchContext);

    const handleSave = () => {
        if (dispatch === null)
            return;
        if (newProfile.name != profile.name && profiles.find(({ name }) => name === newProfile.name)) {
    ...
    return (
      <div className="flex flex-col mx-auto edit-profile px-4 xs:px-14 sm:px-20" style={{ maxWidth: '700px' }}>
        ...
        <div className="divider py-4 mt-2 flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-2 divider pt-2">
                ...
                <div className="flex items-center gap-2 select-none" onClick={() => handleCheck('read_next_episode')}>
                    <Checkbox checked={newProfile.read_next_episode} />
                ...
                <div className="flex items-center gap-2 select-none" onClick={() => handleCheck('read_preview')}>
                    <Checkbox checked={newProfile.read_preview} />
        ...
        <div className="divider pb-2 mt-2 flex flex-wrap py-6 gap-5">
            ...
            <button className="button-classic" onClick={() => handleDelete()}>Supprimer le profil</button>
        </div>
      </div>
    );
}
